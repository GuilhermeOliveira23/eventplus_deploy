# Backend - API EventPlus (instruções para o backend)
...
```
3. Observação: a aplicação lê `builder.Configuration.GetConnectionString("EventPlus")`. Variáveis de ambiente (`ConnectionStrings__EventPlus`) ou user-secrets podem sobrescrever esse valor.
...
```
- Variável de ambiente (sessão PowerShell):
```powershell
...
Execute no diretório do projeto (onde está o .csproj).
...
```
- Via Visual Studio: abra solução e execute com perfil https ou IIS Express.

Abra o Swagger: https://localhost:7209/swagger (ajuste a porta conforme saída do `dotnet run`).
...
## 7) Problemas comuns
- Se a aplicação usar uma connection string diferente, verifique variáveis de ambiente e reinicie o VS/terminal.
...
## 8) Segurança
- Nunca comite `appsettings.json` com senha. Prefira `appsettings.json.template`, `dotnet user-secrets` ou variáveis de ambiente.
